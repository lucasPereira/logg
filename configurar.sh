...
git config --global alias.logf "log --pretty=format:'%C(auto)%h %Cred%ad%Creset%C(auto) %Cblue%an%Creset%C(auto)%d %s' --name-only --topo-order --date=short HEAD master"
...
git config --global user.email `hostname`@`whoami`
